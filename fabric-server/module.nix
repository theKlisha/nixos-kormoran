...
let
  cfg = config.services.fabric-server;

  package = self.packages.${system}.fabric-server;
...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.minecraft-server = {
      ...
      serviceConfig = {
        ExecStart = "${package}/bin/minecraft-server";
      ...
      preStart = ''
        mkdir -p ${cfg.dataDir}
        ln -snf ${eulaFile} eula.txt
        ln -snf ${package}/lib/minecraft/mods ./mods
        ln -snf ${whitelistFile} whitelist.json
    ...
    networking.firewall = lib.mkIf cfg.openFirewall ({
      ...
      allowedTCPPorts = [ serverPort ] ++ lib.optional (queryPort != null) queryPort;
